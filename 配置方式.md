...
确保已正确安装 tanmi-workspace（`npm install -g tanmi-workspace`）。

### 端口被占用

如果默认端口被占用，可以设置环境变量更换端口：

```bash
HTTP_PORT=19542 tanmi-workspace webui
```
...
2. 运行 `tanmi-workspace webui` 启动 Web 界面查看工作区可视化
