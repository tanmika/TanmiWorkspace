...
配置向导会自动：
- 检测已安装的 AI 工具（Claude Code、Cursor）
- 配置 MCP 服务器
- 添加权限（Claude Code）
- 可选安装智能提醒 Hook
...
**验证安装**：在 Claude Code 中输入 `/mcp` 查看已安装的服务器。

### 方式二：源码安装（适用于自定义或其他平台）
...
| 平台 | 配置文件路径 |
|------|-------------|
| **Claude Code** | `~/.claude.json`（用户级）或项目下 `.mcp.json`（项目级） |
| **Cursor** | `~/.cursor/mcp.json` 或项目下 `.cursor/mcp.json` |
| **Windsurf** | `~/.codeium/windsurf/mcp_config.json` |
| **Cline** | VS Code 设置中的 `cline.mcpServers` |
| **其他工具** | 参考对应工具的 MCP 配置文档 |

> Claude Code 也可使用命令行添加：`claude mcp add tanmi-workspace -s user -- node /path/to/dist/check-node-version.js`

### 配置 MCP 工具权限（Claude Code）

> 用于减少mcp工具权限请求带来的打断，提升对话体验

**推荐方式**：使用 `tanmi-workspace setup` 自动配置，或手动在 `~/.claude/settings.local.json` 中添加：
...
```bash
# 交互式安装
tanmi-workspace-hooks

# 或指定安装类型
tanmi-workspace-hooks --claude-hooks
tanmi-workspace-hooks --cursor-hooks
tanmi-workspace-hooks --all
```
...
| Hook 配置文件 | `~/.claude/settings.json` | `~/.cursor/hooks.json` |
...
确保已运行 `npm run build` 构建项目，且 `dist/check-node-version.js` 文件存在。

### 端口被占用

如果 3000 端口被占用，可以设置环境变量更换端口：

```bash
TANMI_PORT=3010 npm run start:http
```
...
如果你之前使用源码安装，现在想切换到 npm 安装：

```bash
# 1. 安装 npm 包
npm install -g tanmi-workspace

# 2. 运行配置向导（会自动覆盖旧的 MCP 配置）
tanmi-workspace setup
```

**说明：**
- 工作区数据保存在 `~/.tanmi-workspace/`，迁移后数据不会丢失
- Hook 脚本会自动更新
- 迁移完成后可删除旧的源码目录
...
1. 向 AI 说 "使用工作台，帮我实现 xxx 功能" 开始新任务
2. 访问 Web 界面（http://localhost:3000）查看工作区可视化
3. 阅读 [用户指南](docs/user-guide.md) 了解完整工作流程
